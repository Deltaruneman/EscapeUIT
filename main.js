...
function isColliding(x, y, size, rX, rY) {
    ...
    const points = [{x,y},{x:x+size,y},{x,y:y+size},{x:x+size,y:y+size}];
    for (let p of points) {
...
function canMoveTo(nextX, nextY) {
    ...
    for (let p of points) {
        ...
        if (map[row][col] === 4 && hopeCount <= 5) return true;
    }
    return true;
}
...
window.onkeydown = (e) => {
    ...
    if (isPaused && !dodgeKeys && (e.code === 'KeyJ' || e.code === 'Enter')) handleNextStory();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (respawnBtn) respawnBtn.onclick = () => {
...
// ============================================================
//  GAME UPDATE + DRAW  (CHỈ 1 LOOP DUY NHẤT)
// ============================================================
function update() {
...
let dodgeKeys  = false; // placeholder để keydown không crash
    ...
    return new Promise(function(resolve) {
        function tick() {
            ...
            if (i < text.length) {
                ...
                _typingTimer = setTimeout(tick, 40);
            } else {
                isTyping = false;
...
function animateHP() {
    ...
    function step() {
        let dirty = false;
        if (Math.abs(displayBossHP - bossHP) > 0.3)     { displayBossHP   += (bossHP   - displayBossHP)   * 0.13; dirty = true; }
        else displayBossHP = bossHP;
        if (Math.abs(displayPlayerHP - battleHP) > 0.3) { displayPlayerHP += (battleHP - displayPlayerHP) * 0.13; dirty = true; }
        ...
        // Đổi màu HP theo lượng còn lại
        if (pFill) pFill.style.background = battleHP < 30 ? '#ff4400' : battleHP < 60 ? '#ffaa00' : 'lime';
...
function bossHitAnim() {
    ...
    setTimeout(() => { el.style.animation = 'pkm-float 3s ease-in-out infinite'; }, 520);
}
...
function buildBattleUI() {
    ...
    bs.innerHTML = `
    <div id="pkm-scene" style="flex:1;position:relative;background:linear-gradient(180deg,#1a1a2e 0%,#16213e 60%,#0f3460 100%);overflow:hidden;">

      <!-- Boss stat box (top-left) -->
      <div style="position:absolute;top:24px;left:36px;background:rgba(0,0,0,0.75);border:2px solid #fff;border-radius:4px;padding:8px 14px;min-width:200px;">
        <div style="font-size:1rem;font-weight:bold;letter-spacing:2px;margin-bottom:4px;">GIÁM THỊ UIT</div>
        <div style="font-size:.75rem;color:#aaa;margin-bottom:5px;">Lv.99</div>
        <div style="background:#333;height:10px;border-radius:5px;overflow:hidden;margin-bottom:3px;">
          <div id="pkm-boss-hp-fill" style="height:100%;width:100%;background:#ff4444;border-radius:5px;"></div>
        </div>
        <div id="pkm-boss-hp-num" style="font-size:.75rem;color:#ff4444;text-align:right;">200/200</div>
      </div>

      <!-- Boss sprite (top-right) -->
      <div style="position:absolute;right:60px;top:18px;text-align:center;">
        <img id="pkm-boss-img" src="boss.png"
             style="height:210px;filter:drop-shadow(0 0 16px red);animation:pkm-float 3s ease-in-out infinite;"
      ...
      <div style="position:absolute;left:70px;bottom:22px;font-size:4rem;line-height:1;">🧑‍🎓</div>

      <!-- Player stat box (bottom-right) -->
      <div style="position:absolute;bottom:18px;right:36px;background:rgba(0,0,0,0.75);border:2px solid #fff;border-radius:4px;padding:8px 14px;min-width:200px;">
        <div style="font-size:1rem;font-weight:bold;letter-spacing:2px;margin-bottom:4px;">SINH VIÊN</div>
        <div style="font-size:.75rem;color:#aaa;margin-bottom:5px;">Lv.4 (Năm 4)</div>
        <div style="background:#333;height:10px;border-radius:5px;overflow:hidden;margin-bottom:3px;">
          <div id="pkm-player-hp-fill" style="height:100%;width:100%;background:lime;border-radius:5px;"></div>
        </div>
    ...
    <div style="background:#111;border-top:3px solid #fff;display:flex;height:160px;box-sizing:border-box;">
      <div style="flex:1;padding:14px 20px;display:flex;align-items:center;">
        <div id="battle-dialog" style="font-size:1.1rem;color:#fff;line-height:1.6;max-width:480px;">* ...</div>
      </div>
    ...
    <style>
      @keyframes pkm-float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-14px)} }
      @keyframes pkm-shake { 0%,100%{transform:translateX(0)} 20%,60%{transform:translateX(-10px)} 40%,80%{transform:translateX(10px)} }
      @keyframes pkm-hit   { 0%,100%{opacity:1;filter:drop-shadow(0 0 16px red)} 50%{opacity:.15;filter:brightness(6) saturate(0)} }
      .pkm-btn { background:#000;color:#fff;border:none;font-family:'Courier New',monospace;font-size:1rem;font-weight:bold;cursor:pointer;padding:0 10px;transition:.15s; }
...
window.startBossBattle = function() {
    ...
    typeDialog('* ...Mày nghĩ lấy đủ 4 chìa khóa là thoát được sao?')
        .then(() => wait(600))
        .then(() => typeDialog('* TA là UIT. Ta trường tồn. Và ngươi... sẽ ở lại đây MÃI MÃI.'))
        .then(() => wait(400))
        .then(() => { showMenu(true); isPlayerTurn = true; });
};

// ---- BOSS MOVES ----
const BOSS_MOVES = [
    [
        { dmg:[15,25], msg:'* Giám thị trừ điểm thành phần! Bạn mất {dmg} HP.' },
        { dmg:[10,18], msg:'* "Lần sau tôi đình chỉ thi!" Bạn mất {dmg} HP.' },
        { dmg:[8, 14], msg:'* Giám thị nhìn bạn chằm chằm. Bạn run lên mất {dmg} HP.' },
    ],
    [
        { dmg:[20,30], msg:'* "Đình chỉ thi toàn bộ!" Bạn mất {dmg} HP.' },
        { dmg:[18,26], msg:'* Giám thị tịch thu bài của bạn! Mất {dmg} HP.' },
        { dmg:[22,32], msg:'* "Ra ngoài ngay!" Bạn bị đuổi, mất {dmg} HP.' },
    ],
    [
        { dmg:[28,40], msg:'* "ĐUỔI HỌC!" — đòn chí mạng! Mất {dmg} HP.' },
        { dmg:[25,35], msg:'* Giám thị đe dọa hủy bằng tốt nghiệp! Mất {dmg} HP.' },
        { dmg:[30,42], msg:'* Tất cả môn điểm F! Bạn tuyệt vọng, mất {dmg} HP.' },
    ]
];
...
window.battleAction = async function(action) {
    ...
    if (action === 'FIGHT') {
        ...
        updateBossPhase(); bossHitAnim(); animateHP();
        const msgs = [
    ...
    // Lượt boss
    const phaseMsg = ['* Giám thị bình tĩnh lại và tấn công!','* Giám thị bắt đầu tức giận!','* GIÁM THỊ MẤT KIỂM SOÁT!'];
    ...
    if (bossPhaseIndex === 1 && bossHP <= 100 && bossHP > 95)
        ...
        const waitMsgs = ['* Lượt của bạn. Hãy chiến đấu!','* Tiếp tục đi, đừng bỏ cuộc!','* Còn sống là còn hy vọng!'];
