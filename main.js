...
let soul = { x: 200, y: 200, size: 12, speed: 4, _hitFlash: 0 };
let bullets = [];
let dodgeTimer = 0;
let dodgeDuration = 0;
let dodgeDamage = 0;
...
const DODGE_BOX = { x: 250, y: 220, w: 300, h: 200 };
...
function draw() {
    ...
    const map = getMap(currentRoomX, currentRoomY);
    if(!map) return; // Bảo vệ lỗi chưa tải xong map
    for(let r=0; r<ROWS; r++){
        for(let c=0; c<COLS; c++){
            if(map[r] && map[r][c]===1) { ctx.fillStyle="#121212"; ctx.fillRect(c*50, r*50, 50, 50); }
            if(map[r] && map[r][c]===3) { ctx.fillStyle="yellow"; ctx.beginPath(); ctx.arc(c*50+25, r*50+25, 10, 0, 7); ctx.fill(); }
            if(map[r] && map[r][c]===5) { ctx.fillStyle="cyan"; ctx.beginPath(); ctx.arc(c*50+25, r*50+25, 8, 0, 7); ctx.fill(); }
            if(map[r] && map[r][c]===4) { ctx.fillStyle="#003300"; ctx.fillRect(c*50, r*50, 50, 50); }
        }
    }
...
window.onload = () => {
    ...
    requestAnimationFrame(loop); // FIX: Chỉ bắt đầu game loop sau khi toàn bộ tải xong
};
...
function typeDialog(text) {
    return new Promise((resolve) => {
        const dialogEl = document.getElementById('battle-dialog');
        if (!dialogEl) { return resolve(); }
...
// FIX: Hàm animateHP đã được viết lại để chạy mượt mà theo từng Frame
function animateHP() {
...
function dodgeLoop() {
    ...
    bullets = bullets.filter(b => {
        ...
        if (!hitThisFrame && Math.hypot(b.x - soul.x, b.y - soul.y) < (b.size/2 + soul.size/2 - 2)) {
            hitThisFrame = true;
            soul._hitFlash = 8;
        }
        return true;
    ...
    if (hitThisFrame) {
        ...
        // FIX: Xóa việc gán ngay lập tức displayPlayerHP = battleHP ở đây
        // Để animateHP tự tính toán giúp thanh máu tụt dần 
        animateHP();
    }
...
function showBattleMenu(show) {
    ...
    if (menu) menu.style.visibility = show ? 'visible' : 'hidden';
}
